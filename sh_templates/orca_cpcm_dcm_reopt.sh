...
cat "$LIST_FILE" | parallel -j "$JOBS_PER_NODE" '
  ...
  echo "Running ORCA REOPT on $INFILE in $OUTDIR (OMP_NUM_THREADS=$OMP_NUM_THREADS)"
  cd "$OUTDIR" || exit 1
...
echo "ORCA reopt batch finished with RC=$RC."
exit $RC
