...
def classify_molecule_for_opt(
    mol_dir: Path,
    outdir_name: str,
    resubmit_failed: bool,
) -> bool:
    """
    Decide if this molecule should be in the todo list.

    Rules:
      - if crest_best.xyz is missing => skip (cannot run).
      - if metadata.json missing/unreadable => include (needs run).
      - if metadata.json has success == False => include only if resubmit_failed=True.
      - if metadata.json has success == True => skip.
    """
    crest_xyz = mol_dir / "crest_best.xyz"
    if not crest_xyz.is_file():
        return False
...
def prepare_orca_input_for_mol(
    ...
    *,
    ...
    """
    Create / overwrite the ORCA .in file for this molecule.
    Returns the path to the .in file.
    """
    mol_name = mol_dir.name
    xyz_path = mol_dir / "crest_best.xyz"
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Submit ORCA B97-3c CPCM(dcm) geometry optimizations from "
            "downselected/molXXXX/crest_best.xyz using node-level parallelism."
        )
    )
    ap.add_argument(
        "--downselection-path",
        required=True,
        help="Directory containing molXXXX subdirs with crest_best.xyz (e.g. downselected)",
    )
    ap.add_argument(
        "--templates",
        default="sh_templates",
        help="Folder with orca_cpcm_dcm_opt.sh template (default: sh_templates)",
    )
    ap.add_argument(
        "--outdir-name",
        default="opt_b97_3c",
        help="Subdirectory name for ORCA inputs/outputs (default: opt_b97_3c)",
    )
    ap.add_argument(
        "--in-template",
        default="in_templates/orca_cpcm_dcm_opt.in",
    ...
    ap.add_argument(
        ...
        type=int,
        default=16,
        help="OpenMP threads / %pal nprocs per ORCA job (default: 16)",
    )
    ap.add_argument(
        "--jobs-per-node",
        type=int,
        default=12,
        help="How many ORCA jobs to run in parallel on a 192-core node (default: 12)",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Also include molecules whose metadata.json exists and has success=false. "
            "For those, the existing opt_b97_3c directory is renamed to "
            "opt_b97_3c_failed_YYYYMMDD_HHMMSS and a fresh opt_b97_3c is created."
        ),
    )
    ...
    sh_template_path = tmpl_dir / "orca_cpcm_dcm_opt.sh"
    ...
    # Build todo list based on metadata.json and crest_best.xyz
    todo = []
    for mol_dir in mol_dirs_all:
        if classify_molecule_for_opt(
            mol_dir,
    ...
    if not todo:
        print(
            f"No molecules to submit under {root} "
            f"(resubmit_failed={args.resubmit_failed})."
        )
        return

    print(
        f"Preparing ORCA opt for {len(todo)} molecules under {root}\n"
        f"  outdir        = '{args.outdir_name}'\n"
        ...
        if args.resubmit_failed and outdir.is_dir():
            ...
            if failed_run:
                ...
                try:
                    ...
                    print(f"[INFO] {mol_dir.name}: moved {args.outdir_name} -> {backup.name}")
                except Exception as e:
                    print(f"[WARN] {mol_dir.name}: could not rename {outdir.name}: {e}")
                    # Skip this molecule to avoid mixing half-renamed state
                    continue

        # Now (re)create a clean outdir and .in file
        try:
    ...
    print(
        ...
        f"each with up to {jobs_per_node} ORCA tasks in parallel."
    )

    for idx, chunk in enumerate(chunks):
        chunk_id = f"{idx:03d}"
        list_file = root / f"orca_opt_chunk_{chunk_id}.txt"
        list_file.write_text(
            "\n".join(str(m.resolve()) for m in chunk) + "\n"
        )

        job_name = f"orca_opt_{chunk_id}"
        sbatch_path = root / f"orca_opt_chunk_{chunk_id}.sbatch"
        write_job_script(
            sh_template_text,
            sbatch_path,
            list_file=list_file,
        ...
        (root / f"orca_opt_chunk_{chunk_id}_submit.json").write_text(
...
if __name__ == "__main__":
    ...
    python submit_orca_opt.py --downselection-path downselected \
        --threads-per-job 16 \
        --jobs-per-node 12 \
        --time 08:00:00 \
        --charge 0 --mult 1

    To resubmit only failed molecules (success == false in metadata.json),
    renaming old opt_b97_3c -> opt_b97_3c_failed_YYYYMMDD_HHMMSS:
    ...
    If they still fail, we can use the other configuration with 
    slower convergence settings:

    python src/submit/submit_orca_opt.py --downselection-path downselected \
        --threads-per-job 192 \
        --in-template in_templates/orca_cpcm_dcm_opt_slow.in \
        --jobs-per-node 1 \
        --time 08:00:00 \
        --charge 0 --mult 1 \
        --resubmit-failed
