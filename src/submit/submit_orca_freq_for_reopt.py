...
def reopt_succeeded(reopt_dir: Path) -> bool:
    """
    Check if the geometry reoptimisation in reopt_dir succeeded,
    based on reopt_dir/metadata.json written by the reopt step.
    """
    meta_path = reopt_dir / "metadata.json"
...
def classify_molecule_for_freq_reopt(
    mol_dir: Path,
    reopt_outdir_name: str,
    freq_outdir_name: str,
    resubmit_failed: bool,
) -> bool:
    """
    Decide if this molecule should be in the todo list for frequency on reoptimised geometries.

    Rules:
      - If reopt metadata missing or reopt NOT successful => skip.
      - If freq_for_reopt/metadata.json missing => include (needs first run).
      - If freq_for_reopt/metadata.json success == True => skip (already done).
      - If freq_for_reopt/metadata.json success == False => include only if resubmit_failed=True.
    """
    reopt_dir = mol_dir / reopt_outdir_name
    if not reopt_succeeded(reopt_dir):
        # Either no metadata, or reopt failed/not run.
        return False
...
def choose_reopt_xyz(reopt_dir: Path, mol_name: str) -> Path:
    """
    Choose the geometry .xyz file for the frequency calculation from the reopt directory.

    Strategy:
      1. Prefer a file named {mol_name}.xyz if present (final optimised geometry).
      2. Otherwise, if exactly one .xyz file, use it.
      3. Otherwise, raise a RuntimeError.
    """
    cand_named = reopt_dir / f"{mol_name}.xyz"
    if cand_named.is_file():
        return cand_named

    xyzs = sorted(reopt_dir.glob("*.xyz"))
    if len(xyzs) == 1:
        return xyzs[0]

    raise RuntimeError(
        f"Cannot uniquely determine reoptimised geometry .xyz in {reopt_dir} "
...
def prepare_orca_input_for_mol(
    ...
    *,
    ...
    using the reoptimised geometry from reopt_b97_3c.
    ...
    # Choose geometry from reopt directory
    xyz_path = choose_reopt_xyz(reopt_dir, mol_name)
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Submit ORCA B97-3c CPCM(dcm) frequency calculations on reoptimised "
            "geometries from downselected/molXXXX/reopt_b97_3c/*.xyz, writing to "
            "freq_b97_3c_for_reopt, using node-level parallelism."
        )
    )
    ap.add_argument(
        "--downselection-path",
    ...
    ap.add_argument(
        ...
        help="Folder with orca_cpcm_dcm_freq_for_reopt.sh template (default: sh_templates)",
    )
    ap.add_argument(
        "--reopt-outdir-name",
        default="reopt_b97_3c",
        help="Directory name with ORCA reoptimisation results (default: reopt_b97_3c)",
    )
    ap.add_argument(
        "--outdir-name",
        default="freq_b97_3c_for_reopt",
        help=(
            "Subdirectory name for ORCA freq inputs/outputs based on reopt geometries "
            "(default: freq_b97_3c_for_reopt)"
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Also include molecules whose freq_b97_3c_for_reopt metadata.json "
            "exists and has success=false. For those, the existing "
            "freq_b97_3c_for_reopt directory is renamed to "
            "freq_b97_3c_for_reopt_failed_YYYYMMDD_HHMMSS and a fresh "
            "freq_b97_3c_for_reopt is created."
        ),
    )
    ...
    sh_template_path = tmpl_dir / "orca_cpcm_dcm_freq_for_reopt.sh"
    ...
    # Build todo list based on reopt metadata and freq_for_reopt metadata
    todo = []
    for mol_dir in mol_dirs_all:
        if classify_molecule_for_freq_reopt(
            mol_dir,
            reopt_outdir_name=args.reopt_outdir_name,
    ...
    if not todo:
        print(
            f"No molecules to submit freq_for_reopt under {root} "
            f"(resubmit_failed={args.resubmit_failed})."
        )
        return

    print(
        f"Preparing ORCA freq (reopt geometries) for {len(todo)} molecules under {root}\n"
        f"  reopt dir     = '{args.reopt_outdir_name}'\n"
    ...
    # Ensure inputs exist; handle resubmit_failed renaming for freq_b97_3c_for_reopt
    prepared = []
    for mol_dir in todo:
        mol_name = mol_dir.name
    ...
    print(
        ...
        f"each with up to {jobs_per_node} ORCA freq_for_reopt tasks in parallel."
    )

    for idx, chunk in enumerate(chunks):
        chunk_id = f"{idx:03d}"
        list_file = root / f"orca_freq_reopt_chunk_{chunk_id}.txt"
        list_file.write_text(
            "\n".join(str(m.resolve()) for m in chunk) + "\n"
        )

        job_name = f"orca_freq_reopt_{chunk_id}"
        sbatch_path = root / f"orca_freq_reopt_chunk_{chunk_id}.sbatch"
        write_job_script(
            sh_template_text,
            sbatch_path,
            list_file=list_file,
        ...
        (root / f"orca_freq_reopt_chunk_{chunk_id}_submit.json").write_text(
...
if __name__ == "__main__":
    ...
    python src/submit/submit_orca_freq_for_reopt.py --downselection-path downselected \
        --threads-per-job 32 \
        --jobs-per-node 6 \
        --time 08:00:00 \
        --charge 0 --mult 1

    To resubmit only failed reopt-freq molecules (success == false in metadata.json),
    renaming old freq_b97_3c_for_reopt -> freq_b97_3c_for_reopt_failed_YYYYMMDD_HHMMSS:
