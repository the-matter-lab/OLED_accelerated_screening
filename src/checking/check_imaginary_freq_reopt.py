...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "Check ORCA frequency runs (freq_b97_3c_for_reopt) under downselected/molXXXX "
            "for imaginary frequencies after reoptimization."
        )
    )
    ap.add_argument(
        "--root",
        required=True,
        help="Directory containing molXXXX/freq_b97_3c_for_reopt (e.g. downselected)",
    )
    ap.add_argument(
        "--outdir-name",
        default="freq_b97_3c_for_reopt",
        help="Subdirectory name used for ORCA freq runs (default: freq_b97_3c_for_reopt)",
    )
    ...
    for mol_dir in sorted(root.iterdir()):
        ...
        if not run_dir.is_dir():
            # Many molXXXX will not have a reopt freq directory; just skip quietly
            continue
        ...
        # "largest" imaginary: these are negative; take the most negative value
    ...
    print("\n===== ORCA freq_b97_3c_for_reopt IMAGINARY FREQUENCY CHECK =====\n")
...
if __name__ == "__main__":
    """
    Example:
    python src/checking/check_imaginary_freq_reopt.py --root downselected
    """
    main()
