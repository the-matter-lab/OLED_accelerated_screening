...
def submit(job_path: Path, workdir: Path, dry_run: bool) -> int:
    if dry_run:
        ...
        return 0
    res = subprocess.run(["sbatch", job_path.name], cwd=workdir, capture_output=True, text=True)
    if res.returncode != 0:
        print(res.stdout)
...
def std2_run_ok(std2_dir: Path) -> bool:
    ...
    sing_ok = False
    trip_ok = False
    xtb = std2_dir / "xtb4stda.out"
    s1  = std2_dir / "std2_singlets.out"
    t1  = std2_dir / "std2_triplets.out"
    try:
        if xtb.is_file() and " 1  SCC done." in xtb.read_text(errors="ignore"):
            xtb_ok = True
    except Exception:
        pass
    try:
        if s1.is_file() and "sTDA done." in s1.read_text(errors="ignore"):
            sing_ok = True
    except Exception:
        pass
    try:
        if t1.is_file() and "sTDA done." in t1.read_text(errors="ignore"):
            trip_ok = True
    except Exception:
        pass
...
def main():
    ap = argparse.ArgumentParser(description="Submit std2 (singlets+triplets) for CREST-completed molXXXX in a partition.")
    ...
    ap.add_argument("--cpus-per-task", type=int, default=40, help="cpus-per-task (default 40)")
    ...
    ap.add_argument("--emax", type=float, default=10.0, help="Energy window for std2 -e (eV, default: 10.0)")
    ap.add_argument("--resubmit", action="store_true", help="Force resubmission even if successful")
    ...
    print(f"Submitting std2 (singlets+triplets) in {part_dir}")
    ...
    for mol_dir in mol_dirs:
        ...
        # Skip if already succeeded unless --resubmit
        if std2_run_ok(std2_dir) and not args.resubmit:
            print(f"[SKIP] {mol_name}: std2 singlets+triplets already completed successfully")
            continue
...
if __name__ == "__main__":
    ...
    python submit_std2.py --partition_dir compounds/partition_0 \
        --gbsa ch2cl2 --emax 10.0 --time 01:00:00
	"""
